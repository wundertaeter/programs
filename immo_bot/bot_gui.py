...
class gui(object):
    ...
    def run(self):
        ...
        Button(self.data['Directory Open']['frame'], text=b_text, command=self.open_dir).pack(side=LEFT)
        ...
        if self.entries['initialdir'] == '/':
            ...
            b_text = self.entries['initialdir'].split('/')[-1]
        Button(self.data['Directory Save']['frame'], text=b_text, command=self.save_file).pack(side=LEFT)
